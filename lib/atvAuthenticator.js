...
var createCipheriv = require('browserify-cipher/browser').createCipheriv
...
function verifier(a)
{
    ...
    const header    = bufferUtil.bufferFrom([0x01, 0x00, 0x00, 0x00]);
    const a_pub_buf = bufferUtil.bufferFrom(a_pub(a), 'hex');

    return {
        verifierBody: Buffer.concat(
            [header, keyPair.public, a_pub_buf].map(function(el) { return bufferUtil.bufferFrom(el) }),
...
function signature(shared, atv_data, signed)
{
    ...
    let encrypted = cipher.update(bufferUtil.bufferFrom(signed, 'hex'), null, 'hex');
    encrypted += cipher.final('hex');
