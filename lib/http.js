...
const HttpMessage = (parseStartLine, writeStartLine) => {
    ...
    instance.write = messageObject => {
        ...
        {
            messageObject.headers['Content-Length'] = bufferUtil.bufferLength(messageObject.body);
        }
        ...
        const buffer = bufferUtil.bufferFrom(messageString);
        ...
        return Buffer.concat([buffer, messageObject.body].map(function(el) { return bufferUtil.bufferFrom(el) }), buffer.length + messageObject.body.length);
    };
    ...
    connect(host, port = 80)
    {
        ...
        return new Promise(resolve => {
            ...
            this.socket.on('data', data => {
                if (data && data.toString() && data.toString().includes('500 Internal Server Error')) {
                    const rr = this.resolveQueue.shift();
                    rr.reject(new Error('500 Internal Server Error'));
                    return
                }
                if (!this.pendingResponse)
                {
